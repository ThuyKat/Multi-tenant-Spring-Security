...
//            String password = jsonNode.get("password").asText();
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		try {
			...
			// Generate JWT access token on successful authentication
			...
			userInfo.put("roles", userDetails.getAuthorities().stream().map(authority -> authority.getAuthority())
...
//        response.addCookie(accessCookie);
			...
			// Clear tenant context to prevent thread contamination
			TenantContext.clear();
		}
